      ...
      <div className="navbar sticky top-0 z-50 bg-base-100/80 backdrop-blur-md border-b border-base-300">
        <div className="navbar-start">
          <div className="dropdown">
            ...
            {mobileOpen && (
              <ul className="menu menu-sm dropdown-content bg-base-200 rounded-box z-[1] mt-3 w-52 p-2 shadow-lg border border-base-300">
                {navItems.map((item) => (
                  <li key={item.path}>
                    <Link
                      to={item.path}
          ...
          <Link to="/" className="btn btn-ghost text-xl font-mono">
            <span className="text-primary">~</span>/yuy<span className="text-base-content/40">{currentPath}</span>
          </Link>
        </div>
          ...
          <ul className="menu menu-horizontal px-1 gap-1">
            {navItems.map((item) => (
              <li key={item.path}>
                <Link
                  to={item.path}
                  className={`${isActive(item.path) ? 'menu-active' : ''} text-sm`}
                >
                  {item.label}
                </Link>
              </li>
      ...
      <footer className="footer sm:footer-horizontal bg-base-200 border-t border-base-300 px-6 py-6 text-base-content/50 text-sm">
        <div className="flex flex-wrap justify-between items-center w-full max-w-5xl mx-auto">
          ...
          <div className="flex gap-4">
            ...
            <a href="mailto:c4llm3Jade@gmail.com" className="link link-hover">
              Gmail
            </a>
          </div>
        </div>
